...
impl MarkdownStyle {
    ...
    fn with_preview_overrides(mut self, colors: &theme::ThemeColors) -> Self {
        ...
        self.heading_level_styles = Some(HeadingLevelStyles {
            h1: Some(TextStyleRefinement {
                font_size: Some(rems(1.65).into()),
                ..Default::default()
            }),
            h2: Some(TextStyleRefinement {
                font_size: Some(rems(1.42).into()),
                ..Default::default()
            }),
            h3: Some(TextStyleRefinement {
                font_size: Some(rems(1.18).into()),
                ..Default::default()
            }),
            h4: Some(TextStyleRefinement {
...
impl MarkdownElement {
    ...
    fn push_markdown_image_fallback(
        &self,
        builder: &mut MarkdownElementBuilder,
        range: &Range<usize>,
        markdown_end: usize,
        ...
        let label = unresolved_image_placeholder_label(dest_url, alt_text);
        ...
        builder.push_text(&label, range.clone());
        builder.pop_div();
    }
                    ...
                    match tag {
                        MarkdownTag::Image { dest_url, .. } => {
                                ...
                                .and_then(|resolve| resolve(dest_url.as_ref()))
                            {
                                ...
                                current_img_block_range = Some(range.clone());
                                self.push_markdown_image_fallback(
                                    &mut builder,
                                    range,
                                    markdown_end,
                                    dest_url.clone(),
...
fn unresolved_image_placeholder_label(
    dest_url: SharedString,
    alt_text: Option<SharedString>,
) -> String {
    let link_label = alt_text
    ...
    format!("Failed to Load: {link_label}")
}
...
fn apply_heading_style(
    ...
    {
        heading = heading.pb_2().border_b_1().border_color(border_color);
    }
...
mod tests {
    ...
    #[test]
    fn test_preview_style_uses_larger_heading_scale_and_borders() {
        let colors = theme::ThemeColors::light();
        let style = MarkdownStyle::default().with_preview_overrides(&colors);
        let heading_styles = style
            .heading_level_styles
            .as_ref()
            .expect("preview markdown should define per-heading styles");

        assert_eq!(style.container_style.text.font_size, Some(rems(1.).into()));
        assert_eq!(
            heading_styles.h1.as_ref().unwrap().font_size,
            Some(rems(1.65).into())
        );
        assert_eq!(
            heading_styles.h2.as_ref().unwrap().font_size,
            Some(rems(1.42).into())
        );
        assert_eq!(
            heading_styles.h3.as_ref().unwrap().font_size,
            Some(rems(1.18).into())
        );
        assert_eq!(style.heading_border_color, Some(colors.border_variant));
    }

    #[gpui::test]
    fn test_default_code_block_controls_are_visible_on_hover(cx: &mut TestAppContext) {
        ensure_theme_initialized(cx);
        let markdown = cx.update(|cx| {
            cx.new(|cx| Markdown::new("```rust\nfn main() {}\n```".into(), None, None, cx))
        });

        let element = MarkdownElement::new(markdown, MarkdownStyle::default());
        match element.code_block_renderer {
            CodeBlockRenderer::Default {
                copy_button_visibility,
                wrap_button_visibility,
                border,
            } => {
                assert_eq!(copy_button_visibility, CopyButtonVisibility::VisibleOnHover);
                assert_eq!(wrap_button_visibility, WrapButtonVisibility::VisibleOnHover);
                assert!(border);
            }
            CodeBlockRenderer::Custom { .. } => panic!("expected default code block renderer"),
        }
    }
    ...
    #[gpui::test]
    fn test_broken_image_link_renders_placeholder_text(cx: &mut TestAppContext) {
        let rendered = render_markdown_with_image_resolver(
        ...
        assert!(
            text.contains("Failed to Load: missing screenshot"),
            "broken image should render a visible placeholder; got: {text:?}"
        );
    }
