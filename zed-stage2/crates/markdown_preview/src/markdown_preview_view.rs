...
use pulldown_cmark::{Event as PulldownEvent, HeadingLevel, Options, Parser, Tag, TagEnd};
...
use ui::{ContextMenu, TintColor, Tooltip, WithScrollbar, prelude::*, right_click_menu};
...
pub struct MarkdownPreviewView {
    ...
    heading_outline: Vec<MarkdownOutlineEntry>,
    table_of_contents_open: bool,
    syncing_scroll: bool,
...
impl MarkdownPreviewView {
    ...
    fn scroll_to_heading_slug(
        ...
        let source_index = self
            .markdown
            .update(cx, |markdown, cx| markdown.scroll_to_heading(slug, cx));
        ...
        {
            Self::move_cursor_to_source_index(&editor, source_index, window, cx);
        }
            ...
            |this, editor, event: &EditorEvent, window, cx| {
                match event {
                    ...
                    EditorEvent::ScrollPositionChanged { local, autoscroll } => {
                        if *local && !*autoscroll {
                            this.sync_editor_scroll_to_preview(editor, window, cx);
                        }
                    }
                    _ => {}
                };
            ...
            let update = view.update(cx, |view, cx| {
                ...
                editor.update(cx, |editor, cx| {
                    let contents = editor
                        ...
                        .as_rope()
                        .to_string();
                    let heading_outline = markdown_heading_outline(&contents);
                    let contents = contents.into();
                    let selection_start = Self::selected_source_index(editor, cx)?;
                    Some((contents, heading_outline, selection_start))
                })
            })?;

            view.update(cx, move |view, cx| {
                if let Some((contents, heading_outline, selection_start)) = update {
                    view.heading_outline = heading_outline;
    ...
    fn sync_preview_to_editor_scroll(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if self.syncing_scroll {
            return;
        }

        let Some(source_index) = self.source_index_for_preview_scroll(cx) else {
            return;
        };
        let Some(editor) = self
            .active_editor
        ...
        editor.update(cx, |editor, cx| {
            let display_snapshot = editor.display_snapshot(cx);
            let row = MultiBufferOffset(source_index)
                .to_display_point(&display_snapshot)
                .row();
            editor.set_scroll_top_row(row, window, cx);
        });
        self.syncing_scroll = false;
    }

    fn sync_preview_to_editor_scroll_after_wheel(
        &mut self,
        _: &ScrollWheelEvent,
        window: &mut Window,
        cx: &mut Context<Self>,
        ...
        window.defer(cx, move |window, cx| {
            if let Some(view) = view.upgrade() {
                view.update(cx, |view, cx| {
                    view.sync_preview_to_editor_scroll(window, cx);
                });
            }
        });
    }

    fn sync_editor_scroll_to_preview(
        &mut self,
        editor: &Entity<Editor>,
        window: &mut Window,
        cx: &mut Context<Self>,
        ...
        let Some(source_index) = Self::top_source_index(editor, cx) else {
            return;
        };
    ...
    fn source_index_for_preview_scroll(&self, cx: &App) -> Option<usize> {
        let max_scroll = self.scroll_handle.max_offset().y;
        if max_scroll <= px(0.) {
            return Some(0);
        }

        let ratio = (-self.scroll_handle.offset().y / max_scroll).clamp(0., 1.);
        let source = self.markdown.read(cx).source().to_string();
        Some(source_index_for_line_ratio(&source, ratio))
    }

    fn top_source_index(editor: &Entity<Editor>, cx: &mut App) -> Option<usize> {
        editor.update(cx, |editor, cx| {
            let display_snapshot = editor.display_snapshot(cx);
            let scroll_top = editor.scroll_position(cx).y.max(0.);
            let top_display_point = DisplayPoint::new(DisplayRow(scroll_top.floor() as u32), 0);
            let top_point = top_display_point.to_point(&display_snapshot);
            let buffer = editor.buffer().read(cx).as_singleton()?;
            let buffer_id = buffer.read(cx).remote_id();
            let (buffer_snapshot, buffer_offset) = display_snapshot
                .buffer_snapshot()
                .point_to_buffer_offset(top_point)?;
    ...
    fn scroll_page_up(&mut self, _: &ScrollPageUp, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.sync_preview_to_editor_scroll(window, cx);
        cx.notify();
    }
    ...
    fn toggle_table_of_contents(
        &mut self,
        _: &gpui::ClickEvent,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
        self.table_of_contents_open = !self.table_of_contents_open;
        cx.notify();
    }
    ...
    fn render_toolbar(&self, cx: &mut Context<Self>) -> Div {
        h_flex()
            ...
            .child(
                IconButton::new(
                    "toggle-markdown-preview-table-of-contents",
                    IconName::ListTree,
                )
                .tooltip(Tooltip::text(if self.table_of_contents_open {
                    "Hide Table of Contents"
                } else {
                    "Show Table of Contents"
                }))
                .aria_label("Toggle Table of Contents")
                .aria_expanded(self.table_of_contents_open)
                .toggle_state(self.table_of_contents_open)
                .selected_style(ButtonStyle::Tinted(TintColor::Accent))
                .on_click(cx.listener(Self::toggle_table_of_contents)),
            )
    }

    fn render_table_of_contents(
        &self,
        position: MarkdownPreviewTableOfContentsPosition,
        cx: &mut Context<Self>,
    ) -> AnyElement {
        let mut drawer = v_flex()
            .id("markdown-preview-table-of-contents")
            .w(px(280.))
            .h_full()
            .flex_none()
            .when(
                position == MarkdownPreviewTableOfContentsPosition::Left,
                |this| this.border_r_1(),
            )
            .when(
                position == MarkdownPreviewTableOfContentsPosition::Right,
            ...
            .child(
                h_flex()
                    ...
                    .items_center()
                    .justify_between()
                    .child(Label::new("Table of Contents").size(LabelSize::Small))
                    .child(
                        IconButton::new(
                            "close-markdown-preview-table-of-contents",
                            IconName::Close,
                        )
                        .tooltip(Tooltip::text("Hide Table of Contents"))
                        .aria_label("Hide Table of Contents")
                        .on_click(cx.listener(Self::toggle_table_of_contents)),
                    ),
            );

        if self.heading_outline.is_empty() {
            return drawer
                .child(
                    div()
                        .px_2()
        ...
        let view = cx.entity().downgrade();
        for entry in &self.heading_outline {
            let slug = entry.slug.clone();
            let padding = match entry.depth {
                0 => 0,
                1 => 2,
                _ => 4,
            };
            let selected = self
                .active_source_index
                .is_some_and(|index| entry.source_range.contains(&index));

            drawer = drawer.child(
                h_flex()
                    .id(format!("markdown-preview-table-of-contents-entry-{slug}"))
                    .min_w_0()
                    .w_full()
                    .rounded_md()
                    .px_2()
                    .py_1()
                    .when(padding > 0, |this| match padding {
                        2 => this.pl_4(),
                        _ => this.pl_6(),
                    })
                    .when(selected, |this| {
                    ...
                    .hover(|this| this.bg(cx.theme().colors().element_hover).cursor_pointer())
                    .on_click({
                        let view = view.clone();
                        move |_, window, cx| {
                            if let Some(view) = view.upgrade() {
                                view.update(cx, |view, cx| {
                                    view.scroll_to_heading_slug(&slug, window, cx);
                                });
                            }
                        }
                    })
                    .child(
                        div()
                            .min_w_0()
                            .flex_1()
                            .child(Label::new(entry.text.clone()).size(LabelSize::Small)),
                    ),
            );
        }
...
fn source_index_for_line_ratio(source: &str, ratio: f32) -> usize {
    if source.is_empty() {
        return 0;
    }
    ...
    let target_line = ((total_lines - 1) as f32 * ratio.clamp(0., 1.)).round() as usize;
    if target_line == 0 {
        return 0;
    }
...
pub(crate) struct MarkdownOutlineEntry {
    pub depth: usize,
...
pub(crate) fn markdown_heading_outline(source: &str) -> Vec<MarkdownOutlineEntry> {
    let parser = Parser::new_ext(source, Options::ENABLE_HEADING_ATTRIBUTES);
    let mut outline = Vec::new();
    let mut active_heading: Option<(HeadingLevel, Range<usize>, String)> = None;
    let mut slug_counts = HashMap::<String, usize>::default();

    for (event, range) in parser.into_offset_iter() {
        match event {
            PulldownEvent::Start(Tag::Heading { level, .. }) => {
                active_heading = Some((level, range, String::new()));
            }
            ...
            PulldownEvent::End(TagEnd::Heading(_)) => {
                if let Some((level, source_range, text)) = active_heading.take()
                    && matches!(
                        level,
                        HeadingLevel::H1 | HeadingLevel::H2 | HeadingLevel::H3
                    )
                {
                    let base_slug = util::markdown::generate_heading_slug(&text);
                    let slug = match slug_counts.get_mut(&base_slug) {
                        ...
                        None => {
                            slug_counts.insert(base_slug.to_string(), 0);
                            base_slug.to_string()
                        }
                    };
...
impl Render for MarkdownPreviewView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let markdown_preview_settings = MarkdownPreviewSettings::get_global(cx);
        let table_of_contents_position = markdown_preview_settings.table_of_contents_position;
        let max_width = markdown_preview_settings.max_width;
        div()
            ...
            .min_h_0()
            .bg(bg_color)
            .child(WithRemSize::new(preview_font_size).size_full().child(
                v_flex().size_full().child(self.render_toolbar(cx)).child({
                    ...
                    let scroll_container = div()
                        ...
                        .on_scroll_wheel(
                            cx.listener(Self::sync_preview_to_editor_scroll_after_wheel),
                        )
                        .p_4()
                        .child(
                            div()
                    ...
                    h_flex()
                        ...
                        .min_h_0()
                        .when(
                            self.table_of_contents_open
                                && table_of_contents_position
                                    == MarkdownPreviewTableOfContentsPosition::Left,
                            |this| {
                                this.child(
                                    self.render_table_of_contents(table_of_contents_position, cx),
                                )
                            },
                        )
                        ...
                        .when(
                            self.table_of_contents_open
                                && table_of_contents_position
                                    == MarkdownPreviewTableOfContentsPosition::Right,
                            |this| {
                                this.child(
                                    self.render_table_of_contents(table_of_contents_position, cx),
                                )
...
mod tests {
    ...
    use crate::markdown_preview_view::markdown_heading_outline;
    use crate::markdown_preview_view::resolve_preview_image;
    use crate::markdown_preview_view::source_index_for_line_ratio;
    ...
    #[test]
    fn builds_nested_markdown_heading_outline() {
        let outline = markdown_heading_outline(
            "# Project\n\nIntro\n\n## Install\n\n### Linux\n\n#### Ignored\n\n## Install\n",
        );
        ...
        assert_eq!(
            summary,
            vec![
                (0, "Project".to_string(), "project".to_string(), 0..10),
                (1, "Install".to_string(), "install".to_string(), 18..29),
                (2, "Linux".to_string(), "linux".to_string(), 30..40),
                (1, "Install".to_string(), "install-1".to_string(), 55..66),
            ]
        );
    }

    #[test]
    fn source_index_for_line_ratio_uses_line_starts() {
        let source = "first\nsecond\nthird\nfourth";

        assert_eq!(source_index_for_line_ratio(source, 0.), 0);
        assert_eq!(source_index_for_line_ratio(source, 0.34), 6);
        assert_eq!(source_index_for_line_ratio(source, 0.67), 13);
        assert_eq!(source_index_for_line_ratio(source, 1.), 19);
    }
